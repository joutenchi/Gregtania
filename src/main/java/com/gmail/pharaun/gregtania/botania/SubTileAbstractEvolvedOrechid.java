...
public abstract class SubTileAbstractEvolvedOrechid extends SubTileFunctional {
    ...
    private static final int RANGE = Config.range;
    private static final int RANGE_Y = Config.range_y;
    private static final Set<String> notGregOre = new HashSet<>(1);
    ...
    public abstract Map<String, ItemStack> getOreStackMap();

    public abstract Collection<WeightedRandom.Item> getValues();
    ...
    public ItemStack getOreToPut() {
        ...
        Collection<WeightedRandom.Item> values = getValues();
        if (values.isEmpty() || values.size() != map.size()) {
            for (Map.Entry<String, Integer> entry : map.entrySet()) {
                String key = entry.getKey();
                Integer value = entry.getValue();
                values.add(new StringRandomItem(value, key));
            }
        }

        String ore = ((StringRandomItem) WeightedRandom.getRandomItem(supertile.getWorldObj().rand, values)).s;

        Map<String, ItemStack> oreStackMap = getOreStackMap();
        if (!oreStackMap.containsKey(ore)) {
            ...
            for (ItemStack stack : ores) {
                ...
                if (clname.startsWith("gregtech") || clname.startsWith("gregapi")) // Found, let's find out how to get the NBT tag and spawn it
                {
                    oreStackMap.put(ore, stack);
                    return stack;
                }
            }
            // Didn't find a gregtech ore, return first result
            oreStackMap.put(ore, ores.get(0));
            notGregOre.add(ores.get(0).getItem().getClass().getName());
        }
        return getOreStackMap().get(ore);
    }
